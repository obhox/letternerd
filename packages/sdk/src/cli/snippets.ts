/**
 * The built-in fallback plan.
 *
 * This is a deliberate, tested copy of the region between the same two markers
 * in `packages/capabilities/src/install.ts`. The CMS is the authority: when the
 * CLI is given `--studio-url` and a read key it fetches the real plan and never
 * calls anything here. This exists for the offline case — an agent in a
 * customer's repository with no studio to reach — and a fallback that guessed
 * the SDK's own API would be worse than no fallback at all.
 *
 * It cannot be an import. `@letternerd/sdk` is published to a registry and
 * `@cms/capabilities` is private to the CMS monorepo, so a dependency in that
 * direction is unresolvable on a customer's machine. The guard against drift is
 * `packages/capabilities/src/__tests__/install.test.ts`, which reads both files
 * and fails the build when the marked regions differ by a single byte.
 *
 * Do not edit this file directly. Edit `install.ts` and copy the region across.
 */
